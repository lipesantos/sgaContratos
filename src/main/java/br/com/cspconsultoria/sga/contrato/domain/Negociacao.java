...
public class Negociacao implements Serializable {
    ...
    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JoinTable(name = "negociacao_propostas",
               joinColumns = @JoinColumn(name = "negociacao_id", referencedColumnName = "id"),
               inverseJoinColumns = @JoinColumn(name = "propostas_id", referencedColumnName = "id"))
