import React, { useState, useEffect } from 'react';
import { connect } from 'react-redux';
import { Link, RouteComponentProps } from 'react-router-dom';
import { Button, Row, Col, Label } from 'reactstrap';
import { AvFeedback, AvForm, AvGroup, AvInput, AvField } from 'availity-reactstrap-validation';
import { Translate, translate, ICrudGetAction, ICrudGetAllAction, ICrudPutAction } from 'react-jhipster';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { IRootState } from 'app/shared/reducers';

import { IProposta } from 'app/shared/model/proposta.model';
import { getEntities as getPropostas } from 'app/entities/proposta/proposta.reducer';
import { getEntity, updateEntity, createEntity, reset } from './evento.reducer';
import { IEvento } from 'app/shared/model/evento.model';
import { convertDateTimeFromServer, convertDateTimeToServer, displayDefaultDateTime } from 'app/shared/util/date-utils';
import { mapIdList } from 'app/shared/util/entity-utils';

export interface IEventoUpdateProps extends StateProps, DispatchProps, RouteComponentProps<{ id: string }> {}

export const EventoUpdate = (props: IEventoUpdateProps) => {
  const [propostaId, setPropostaId] = useState('0');
  const [isNew, setIsNew] = useState(!props.match.params || !props.match.params.id);

  const { eventoEntity, propostas, loading, updating } = props;

  const handleClose = () => {
    props.history.push('/evento' + props.location.search);
  };

  useEffect(() => {
    if (isNew) {
      props.reset();
    } else {
      props.getEntity(props.match.params.id);
    }

    props.getPropostas();
  }, []);

  useEffect(() => {
    if (props.updateSuccess) {
      handleClose();
    }
  }, [props.updateSuccess]);

  const saveEntity = (event, errors, values) => {
    values.dataEmissao = convertDateTimeToServer(values.dataEmissao);
    values.dataVencimento = convertDateTimeToServer(values.dataVencimento);

    if (errors.length === 0) {
      const entity = {
        ...eventoEntity,
        ...values,
      };

      if (isNew) {
        props.createEntity(entity);
      } else {
        props.updateEntity(entity);
      }
    }
  };

  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="sgaContratosApp.evento.home.createOrEditLabel">
            <Translate contentKey="sgaContratosApp.evento.home.createOrEditLabel">Create or edit a Evento</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            <p>Loading...</p>
          ) : (
            <AvForm model={isNew ? {} : eventoEntity} onSubmit={saveEntity}>
              {!isNew ? (
                <AvGroup>
                  <Label for="evento-id">
                    <Translate contentKey="global.field.id">ID</Translate>
                  </Label>
                  <AvInput id="evento-id" type="text" className="form-control" name="id" required readOnly />
                </AvGroup>
              ) : null}
              <AvGroup>
                <Label id="dataEmissaoLabel" for="evento-dataEmissao">
                  <Translate contentKey="sgaContratosApp.evento.dataEmissao">Data Emissao</Translate>
                </Label>
                <AvInput
                  id="evento-dataEmissao"
                  type="datetime-local"
                  className="form-control"
                  name="dataEmissao"
                  placeholder={'YYYY-MM-DD HH:mm'}
                  value={isNew ? displayDefaultDateTime() : convertDateTimeFromServer(props.eventoEntity.dataEmissao)}
                />
              </AvGroup>
              <AvGroup>
                <Label id="dataVencimentoLabel" for="evento-dataVencimento">
                  <Translate contentKey="sgaContratosApp.evento.dataVencimento">Data Vencimento</Translate>
                </Label>
                <AvInput
                  id="evento-dataVencimento"
                  type="datetime-local"
                  className="form-control"
                  name="dataVencimento"
                  placeholder={'YYYY-MM-DD HH:mm'}
                  value={isNew ? displayDefaultDateTime() : convertDateTimeFromServer(props.eventoEntity.dataVencimento)}
                />
              </AvGroup>
              <AvGroup>
                <Label id="descricaoLabel" for="evento-descricao">
                  <Translate contentKey="sgaContratosApp.evento.descricao">Descricao</Translate>
                </Label>
                <AvField id="evento-descricao" type="text" name="descricao" />
              </AvGroup>
              <AvGroup>
                <Label id="valorLabel" for="evento-valor">
                  <Translate contentKey="sgaContratosApp.evento.valor">Valor</Translate>
                </Label>
                <AvField id="evento-valor" type="text" name="valor" />
              </AvGroup>
              <AvGroup>
                <Label for="evento-proposta">
                  <Translate contentKey="sgaContratosApp.evento.proposta">Proposta</Translate>
                </Label>
                <AvInput id="evento-proposta" type="select" className="form-control" name="proposta.id">
                  <option value="" key="0" />
                  {propostas
                    ? propostas.map(otherEntity => (
                        <option value={otherEntity.id} key={otherEntity.id}>
                          {otherEntity.id}
                        </option>
                      ))
                    : null}
                </AvInput>
              </AvGroup>
              <Button tag={Link} id="cancel-save" to="/evento" replace color="info">
                <FontAwesomeIcon icon="arrow-left" />
                &nbsp;
                <span className="d-none d-md-inline">
                  <Translate contentKey="entity.action.back">Back</Translate>
                </span>
              </Button>
              &nbsp;
              <Button color="primary" id="save-entity" type="submit" disabled={updating}>
                <FontAwesomeIcon icon="save" />
                &nbsp;
                <Translate contentKey="entity.action.save">Save</Translate>
              </Button>
            </AvForm>
          )}
        </Col>
      </Row>
    </div>
  );
};

const mapStateToProps = (storeState: IRootState) => ({
  propostas: storeState.proposta.entities,
  eventoEntity: storeState.evento.entity,
  loading: storeState.evento.loading,
  updating: storeState.evento.updating,
  updateSuccess: storeState.evento.updateSuccess,
});

const mapDispatchToProps = {
  getPropostas,
  getEntity,
  updateEntity,
  createEntity,
  reset,
};

type StateProps = ReturnType<typeof mapStateToProps>;
type DispatchProps = typeof mapDispatchToProps;

export default connect(mapStateToProps, mapDispatchToProps)(EventoUpdate);
