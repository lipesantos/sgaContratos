...
public class NegociacaoResourceIT {
    ...
    @Mock
    private NegociacaoRepository negociacaoRepositoryMock;
    ...
    public void getAllNegociacaosWithEagerRelationshipsIsEnabled() throws Exception {
        when(negociacaoRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restNegociacaoMockMvc.perform(get("/api/negociacaos?eagerload=true"))
            .andExpect(status().isOk());

        verify(negociacaoRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllNegociacaosWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(negociacaoRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
